...
struct
  ...
  *)
  ...
  let rec record_helper (data_bundle : (string * loc) list)(locs : (loc list) ref)
   : (loc list) ref =
   match data_bundle with
   | (id, l) :: tl ->
    let _ = (locs := l :: !locs) in
    record_helper tl locs
   | [] -> locs

  let rec sweep_S (s : svalue list)(locs : (loc list) ref) : (loc list) ref =
    match s with
    | hd :: tl ->
      (match hd with
      | V (v) ->
        ...
        | L (l) ->
          ...
          sweep_S tl locs
        | R (data_bundle) ->
          let _ = (locs := !(record_helper data_bundle locs)) in
            sweep_S tl locs
        | _ -> sweep_S tl locs )
      | P (f, c, e) ->
        let _ = (locs := !(sweep_E e locs)) in
  ...
  let sweep_offset (m : (loc * value) list) (locs : (loc list) ref)
  : loc list =
    let (base_list, _) = (List.split !locs) in
    let (mem_list, _) = (List.split m) in
    let result = List.filter (fun (base, _) -> List.mem base base_list) mem_list in
    result


  let safe_list_find p l =
    try let _ = List.find p l in true
    with Not_found -> false

  let rec memory_check (m : (loc * value) list) (locs : (loc list) ref)
    : (loc list) ref =
    match m with
    | (location, values) :: tl ->
      let cond = safe_list_find (fun (x)-> x = location) !locs in
      (match values with
        | L (l) ->(
          match cond with
            | true ->
              let _ = (locs := (l :: !locs)) in
              memory_check tl locs
            | false -> memory_check tl locs
            )
        | R (data_bundle) -> (
          match cond with
            | true ->
              let to_add = List.map (fun (id, l)-> l) data_bundle in
              let _ = (locs := (to_add @ !locs)) in
              memory_check tl locs
            | false -> memory_check tl locs
            )
        | _ -> memory_check tl locs)
    | [] -> locs

  let rec print loc_list =
    match loc_list with
    | (base, offset) :: tl ->
      let _ = print_endline ("(" ^ (string_of_int base) ^ ", " ^ (string_of_int offset) ^ ")") in
      print tl
    | _ -> ()

  let rec fixed_point m locs =
    let fixed = !locs in
    let _ = (locs := sweep_offset m locs) in
    let _ = (locs := !(memory_check m locs)) in
    if (fixed = !locs) then locs else fixed_point m locs
  ...
  let malloc_with_gc s m e c k =
    if List.length m < mem_limit then
      ...
      let _ = (reachable_locs := !(sweep_E e reachable_locs)) in
      let _ = (reachable_locs := !(sweep_S s reachable_locs)) in
      let _ = (reachable_locs := !(sweep_K k reachable_locs)) in
      let _ = (reachable_locs := !(fixed_point m reachable_locs)) in

      (* let _ = print (!reachable_locs) in *)
