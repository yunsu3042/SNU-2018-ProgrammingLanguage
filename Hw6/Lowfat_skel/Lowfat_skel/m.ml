...
struct
  ...
  let op2fn =
    ...
    | EQ ->(
      fun (v1,v2) -> match (v1, v2) with
        | (Int int1, Int int2) -> Bool (v1 = v2)
        | (Bool b1, Bool b2) -> Bool (b1 = b2)
        | (String s1, String s2) -> Bool (s1 = s2)
        | (Loc l1, Loc l2) -> Bool (l1 = l2)
        | _ -> raise (TypeError "not a sametype")
        )
  ...
  let rec eval env mem exp =
    ...
    | APP (e1, e2) ->
      ...
      | RecFun (f, x, e) ->
        let env'' = env @+ (x, v2) in
        let env''' = env'' @+ (f, Closure(RecFun(f, x, e), env')) in
        eval env''' m'' e)
    | IF (e1, e2, e3) ->
      let (v1, m') = eval env mem e1 in
    ...
    | ASSIGN (e1, e2) ->
      ...
      let mem''' = store mem'' (getLoc v1, v2) in
      (v2, mem''')
    | BANG (e) ->
      let (v1, mem') = eval env mem e in
      let v2 = load mem' (getLoc v1) in
      (v2, mem')
    | SEQ (e1, e2) ->
      let (v1, mem') = eval env mem e1 in
      let (v2, mem'') = eval env mem' e2 in
